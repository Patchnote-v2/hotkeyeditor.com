import izip
import parse
import strings as hpk_strings

# these are derived from the numerical ids/text ids in the game configs
_hk_names = {k: v[0] for k, v in hpk_strings.hk_mapping.items()}
...
hk_desc = {k: v[1] for k, v in hpk_strings.hk_mapping.items()}

hk_groups = hpk_strings.hk_groups
...
class HotkeyFile:
    def __init__(self, hki, validate=True, file_type="HKI"):
        hk_bytes = izip.decompress(hki)
        parser = parse.HkParser(file_type)
